...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_pair(self, filepath_and_text):
        ...
        text = self.get_text(text, add_blank=self.add_blank) #텍스트를 전처리하여 텐서로 변환
        mel = self.get_mel(filepath) #멜 스펙트로그램으로 변환
        return (text, mel)
...
#배치 데이터를 정렬하고 패딩(padding)하여 동일한 크기의 텐서로 만들어 모델에 입력할
